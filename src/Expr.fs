...
open Tranq.Text
open Tranq.ExprAst
open Tranq.ExprParser

type internal ExprException (message, ?innerException:exn) =
...
exception internal ExprGenerateError of Message * Location * exn option

[<RequireQualifiedAccess>]
[<CompilationRepresentation(CompilationRepresentationFlags.ModuleSuffix)>]
module internal Expr = 

  let startLocation = { pos_bol = 0; pos_fname = "Expr"; pos_cnum = 0; pos_lnum = 1 }

  let getErrorMessage (ctxt:ParseErrorContext<token>) = 
    match ctxt.CurrentToken with
    | None -> 
      SR.TRANQ1014 ctxt.Message
    | Some token -> 
      SR.TRANQ1019 (token_to_string token) 
  ...
  let appendDetail message (text:string) loc = 
     { message with Text = message.Text + " " + (SR.TRANQ1020 (text, loc.pos_lnum, loc.pos_cnum)).Text }

  let parse expr =
    let lexbuf = LexBuffer<char>.FromString(expr)
    lexbuf.EndPos <- { 
      pos_bol = startLocation.pos_bol
      pos_fname = startLocation.pos_fname
      pos_cnum = startLocation.pos_cnum
      pos_lnum = startLocation.pos_lnum }
    try
      ExprParser.start ExprLexer.tokenize lexbuf
    with 
    | ExprParseError (message, loc) -> 
      let message = appendDetail message expr loc
      raise <| ExprException (message)
    | ExprParseErrorWithContext obj ->
    ...
    | ex -> 
      let loc = getLocation lexbuf
      let message = appendDetail (SR.TRANQ1015 ()) expr loc
      raise <| ExprException (message, ex)
  ...
  let (|GetDictValue|_|) (obj:obj, typ, name) =
    if Dict.isDict typ then
      match Dict.getDictValue(name, obj, typ) with
  ...
  let applyAdd (loc:Location) (lhs:obj * Type, rhs:obj * Type) =
    ...
    | _ ->
      ...
      if lhsType <> rhsType then
        raise <| ExprGenerateError (SR.TRANQ1001 ("+", lhsType.FullName, rhsType.FullName), loc, None)
      else
        raise <| ExprGenerateError (SR.TRANQ1002 ("+", lhsType.FullName), loc, None)
  ...
  let applyLessThan (loc:Location) (lhs:obj * Type, rhs:obj * Type) =
    ...
    | _ -> 
      ...
      if lhsType <> rhsType then
        ...
        raise <| ExprGenerateError (SR.TRANQ1007 ("<", lhsType.FullName), loc, None)
  ...
  let applyAndAlso (loc:Location) (lhs:obj * Type, rhs:Lazy<obj * Type>) =
    ...
    | Some result ->
      if result then
        ...
        | _ -> 
          raise <| ExprGenerateError (SR.TRANQ1009 ("&&", (snd rhs.Value).FullName), loc, None)
      else 
        box false, typeof<bool>
    | _ ->
        raise <| ExprGenerateError (SR.TRANQ1009 ("&&", (snd lhs).FullName), loc, None)
  ...
  let applyNot op (loc:Location) (value:obj * Type)  =
    ...
    | _ -> 
      raise <| ExprGenerateError (SR.TRANQ1011 (op, snd value), loc, None)
  ...
  let applyApplication (loc:Location) (func:obj, funcType:Type) (arg:obj, argType:Type) =
    ...
    if invokeMethod = null then
      raise <| ExprGenerateError (SR.TRANQ1024 (funcType.FullName, argType.FullName), loc, None)
    try
      let result = invokeMethod.Invoke(func, [| arg |])
      result, invokeMethod.ReturnType
    with 
    | exn ->
      raise <| ExprGenerateError (SR.TRANQ1025 (funcType.FullName), loc, Some exn)
  ...
  let applyIn (loc:Location) (ident:string) (expr:obj, exprType:Type) =
    ...
    | _ -> 
      raise <| ExprGenerateError (SR.TRANQ1022 (), loc, None)

  let generate inp (env:IDictionary<string, obj * Type>) ast =
    let rec visitExpr = 
      function 
      ...
      | AndAlso (lhs, rhs, loc) -> applyAndAlso loc (visitExpr lhs, lazy(visitExpr rhs))
      | OrElse (lhs, rhs, loc) -> applyOrElse loc (visitExpr lhs, lazy(visitExpr rhs))
      | Not (op, expr, loc) -> visitExpr expr |> applyNot op loc
      ...
      | Application (expr, factor, loc) -> applyApplication loc (visitExpr expr) (visitFactor factor)
      | Tuple (factorList, loc) -> applyNewTuple loc (List.map visitFactor factorList)
      | In (factor, expr, loc) -> 
        let ident =
          match factor with
          | Var (name, _) -> name
          | _ -> raise <| ExprGenerateError (SR.TRANQ1021 (), loc, None)
        let expr = visitExpr expr
    ...
    and visitFactor factor  =
      ...
      | Var (name, loc) -> 
        ...
        | _ -> raise <| ExprGenerateError (SR.TRANQ1006 name, loc, None)
      | Property (expr, propName, loc) -> 
        let value, typ = visitFactor expr
        match (value, typ, propName) with
        | GetNullableProperty result -> result
        | GetOptionProperty result -> result
        | null, _, _ -> 
          raise <| ExprGenerateError (SR.TRANQ1026 (propName, typ.FullName), loc, None)
        | GetDictValue result -> result
        | GetProperty result -> result
        | GetField result -> result
        | _ -> 
          raise <| ExprGenerateError (SR.TRANQ1003 (propName, typ.FullName), loc, None)
      | StaticProperty (typeName, propName, loc) ->
        let typ = 
          match typeName with 
          | GetType typ -> typ 
          | _ -> raise <| ExprGenerateError (SR.TRANQ1005(typeName), loc, None)
        match (typ, propName) with
        | GetStaticProperty result -> result
        | GetStaticField result -> result
        | _ ->  raise <| ExprGenerateError (SR.TRANQ1004 (propName, typ.FullName), loc, None)
      | Parens expr-> visitExpr expr
    ...
    and pair lhs rhs = 
      let lhs = visitExpr lhs
      let rhs = visitExpr rhs
      lhs, rhs
    try
      visitExpr ast
    with
    | ExprGenerateError (message, loc, ex) ->
      let message = appendDetail (message) inp loc
      match ex with
      | Some ex ->
        raise <| ExprException (message, ex)
      | _ ->
  ...
  let cache = ConcurrentDictionary<string, Lazy<ExprAst.Expr>>()

  let evaluate inp env =
    cache.GetOrAdd(inp, Lazy(fun () -> parse inp))
    |> Lazy.force
    |> generate inp env
